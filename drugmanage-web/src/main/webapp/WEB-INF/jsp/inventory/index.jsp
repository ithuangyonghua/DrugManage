	 ...
	 <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
		<div class="container-fluid">
			<div class="navbar-header">
				<div>
					<a class="navbar-brand" style="font-size: 32px;" href="#">库存管理
						- 库存信息</a>
				</div>
			</div>
				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<button type="button" class="btn btn-primary"
							style="float: right;" onclick="window.location.href='${APP_PATH}/inventory/insertInventory'">
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$("#batchDelBtn").click(function(){
		        ...
		        if(boxs.length==0){
		        	...
		        	var index;
					layer.confirm("删除选择的库存信息,是否继续",  {icon: 3, title:'提示'}, function(cindex){
						//第一个代表的是确定按钮
						$.ajax({
							type:"POST",
							url:"${APP_PATH}/inventory/deleteInventoryBouth",
		...
		function pageQuery(pageno) {
			...
			$
					.ajax({
						type : "POST",
						url : "${APP_PATH}/inventory/pageQuery",
						data : jsonData,
						...
						success : function(result) {
							...
							if (result.success == true) {
								...
								//i当前索引,user 当前临时数据
								//shift + alt + a
								$.each(drugData,function(i, inventory) {
									tableContext += '	 <tr>  ';
									tableContext += '    <td>'+(i+1)+'</td>';
								/* 	tableContext += '	  <td><input type="checkbox"  name="id"  value="'+inventory.id+'" </td>'; */
									/* tableContext += '    <td>'+inventory.drugid+'</td>'; */
									tableContext += '    <td>'+inventory.drugInfo.drugname+'</td>';
								    tableContext += '    <td>'+inventory.inventorySum+'</td>'; 
									/* tableContext += '    <td>￥'+inventory.price+'</td>';  */
									
								/* 	if((inventory.yt).length>18){
										tableContext += '    <td>'+(inventory.yt).substring(1,18)+'......</td>';
									}else{
										tableContext += '    <td>'+(inventory.yt)+'</td>';
									} */
									tableContext += '    <td>';
									tableContext += '	      <button type="button" onclick="updateDruginfo(\''+inventory.id+'\')" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
									//tableContext += '		  <button type="button" onclick="deleteInventory(\''+inventory.id+'\',\''+inventory.drugInfo.drugname+'\')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
									tableContext += '	  </td>';
									tableContext += '  </tr>';
								});
                                if(pageno>1){//上一页的判断
		...
		function updateDruginfo(uid){
			window.location.href="${APP_PATH}/inventory/edit?id="+ uid;
			
		}
		
		//删除
		function deleteInventory(uid,drugname){
			var index;
			layer.confirm("删除库存信息【"+drugname+"】,是否继续",  {icon: 3, title:'提示'}, function(cindex){
				//第一个代表的是确定按钮
				$.ajax({
					type:"POST",
					url:"${APP_PATH}/inventory/deleteInventoryById",
					data:{
						"id":uid
					},
					success:function(result){
