    ...
    <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
      <div class="container-fluid">
        <div class="navbar-header">
            <div><a class="navbar-brand" style="font-size:32px;" href="user.html">药品管理 - 库存修改</a></div>
        </div>
			  ...
			  <div class="panel-body">
				<form role="form"  id="updateForm">
				  <div class="form-group">
					...
					<input type="hidden" value="${inventoryInfo.id}" id="id"/>
					<input type="text" class="form-control" id="inventorySum"  value="${inventoryInfo.inventorySum}"  placeholder="请输入库存数量">
				  </div>
            ...
            $(function () {
			    ...
			    $("#updateBtn").click(function(){
			    	 ...
			    	 if(inventorySum==""){
			    		 ...
			    		 if(inventorySum<0){
			    			 layer.msg("库存数量不能为负值,请输入", {time:1000, icon:5, shift:6}, function(){
				    		 });
				    		 return;
			    		 }
			    	 }
			    	 $.ajax({
			    		 ...
			    		 success:function(result){
			    			 ...
			    			 if(result.success){
			    				 layer.msg("库存信息修改成功", {time:1000, icon:6, shift:6}, function(){
			    					 window.location.href="${APP_PATH}/inventory/index"
					    		 });
			    			 }else{
			    				 layer.msg("库存信息修改失败,请重新输入", {time:1000, icon:5, shift:6}, function(){
					    			 //alert("回调方法");
					    		 });
			    			 }
			    		 }
