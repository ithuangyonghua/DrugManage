    ...
    <nav class="navbar navbar-inverse navbar-fixed-top" role="navigation">
      <div class="container-fluid">
        <div class="navbar-header">
            <div><a class="navbar-brand" style="font-size:32px;" href="user.html">药品管理 - 药品修改</a></div>
        </div>
			  ...
			  <div class="panel-body">
				<form role="form" id="DrugForm">
				 <%--  <div class="form-group">
					...
					<input type="text"  class="form-control" value="${drugInfo.drugid}" id="drugid" placeholder="请输入药品编号">
				  </div> --%>
				  <div class="form-group">
				  <input type="hidden" value="${drugInfo.drugid}" id="drugid"/>
					<label for="exampleInputPassword1">药品名称</label>
					<input type="text" class="form-control" id="drugname" value="${drugInfo.drugname}" placeholder="请输入药品名称">
				  </div>
				  <div class="form-group">
					<label for="exampleInputPassword1">药品类别</label>
					<c:set  var="drugcategoryid" value="${drugInfo.drugcategory}" scope="request"/>
					<select class="form-control" id="drugcategory" >
						  <option value="" selected="selected">全部</option>
						  <c:forEach items="${drugCategoryAll}" var="drug">
						   <option <c:if test="${drug.dcategoryid==drugcategoryid}">selected</c:if> value="${drug.dcategoryid}">${drug.name}</option>
						  </c:forEach>
						</select>
				  </div>
				  <div class="form-group">
					<label for="exampleInputPassword1">价格</label>
					<input type="text" class="form-control"  value="${drugInfo.price}" id="price" placeholder="请输入价格">
				  </div>
				  <div class="form-group">
					<label for="exampleInputPassword1">药品用途</label>
					<input type="text" class="form-control"  value="${drugInfo.yt}" id="yt" placeholder="请输入药品用途">
				  </div>
			    	 ...
			    	 $.ajax({
			    		 ...
			    		 success:function(result){
			    			 ...
			    			 if(result.success==true){
			    				 layer.msg("药品信息修改成功", {time:1000, icon:6, shift:6}, function(){
			    					 window.location.href="${APP_PATH}/drug/index"
					    		 });
			    			 }else{
			    				 layer.msg("药品信息修改失败,请重新输入", {time:1000, icon:5, shift:6}, function(){
					    			 //alert("回调方法");
					    			 $("#DrugForm")[0].reset();
					    		 });
			    			 }
